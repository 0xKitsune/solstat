...
contract WardenPledge is Ownable, Pausable, ReentrancyGuard {
    ...
    /** @notice Event emitted when xx */
    event ExtendPledgeDuration(
        uint256 indexed pledgeId,
        uint256 oldEndTimestamp,
        uint256 newEndTimestamp
    );
    /** @notice Event emitted when xx */
    event IncreasePledgeTargetVotes(
        uint256 indexed pledgeId,
        uint256 oldTargetVotes,
        uint256 newTargetVotes
    );
    /** @notice Event emitted when xx */
    event IncreasePledgeRewardPerVote(
        uint256 indexed pledgeId,
     ...
     */
    function getUserPledges(address user)
        external
        view
     ...
     */
    function _getRoundedTimestamp(uint256 timestamp)
        internal
        pure
        returns (uint256)
    {
    ...
    function pledgePercent(
        ...
        uint256 amount = (votingEscrow.balanceOf(msg.sender) * percent) /
            MAX_PCT;
    ...
    function _pledge(
        ...
        if (
            endTimestamp > pledgeParams.endTimestamp ||
            endTimestamp != _getRoundedTimestamp(endTimestamp)
        ...
        if (delegationBoost.allowance(user, address(this)) < amount)
            revert Errors.InsufficientAllowance();
        if (delegationBoost.delegable_balance(user) < amount)
            revert Errors.CannotDelegate();

        // Check that this will not go over the Pledge target of votes
        if (
            delegationBoost.adjusted_balance_of(pledgeParams.receiver) +
                amount >
            pledgeParams.targetVotes
        ) revert Errors.TargetVotesOverflow();
        ...
        uint256 rewardAmount = (totalDelegatedAmount *
            pledgeParams.rewardPerVote) / UNIT;

        if (rewardAmount > pledgeAvailableRewardAmounts[pledgeId])
            revert Errors.RewardsBalanceTooLow();
    ...
    function createPledge(
        ...
        if (minAmountRewardToken[rewardToken] == 0)
            revert Errors.TokenNotWhitelisted();
        if (rewardPerVote < minAmountRewardToken[rewardToken])
        ...
        vars.totalRewardAmount =
            (rewardPerVote * vars.votesDifference * vars.duration) /
            UNIT;
        vars.feeAmount = (vars.totalRewardAmount * protocalFeeRatio) / MAX_PCT;
        if (vars.totalRewardAmount > maxTotalRewardAmount)
            revert Errors.IncorrectMaxTotalRewardAmount();
        if (vars.feeAmount > maxFeeAmount)
            revert Errors.IncorrectMaxFeeAmount();
        ...
        pledgeAvailableRewardAmounts[vars.newPledgeID] += vars
            .totalRewardAmount;
    ...
    function extendPledge(
        ...
        if (
            newEndTimestamp != _getRoundedTimestamp(newEndTimestamp) ||
        ...
        uint256 totalRewardAmount = (pledgeParams.rewardPerVote *
            pledgeParams.votesDifference *
            addedDuration) / UNIT;
        uint256 feeAmount = (totalRewardAmount * protocalFeeRatio) / MAX_PCT;
        if (totalRewardAmount > maxTotalRewardAmount)
            revert Errors.IncorrectMaxTotalRewardAmount();
        if (feeAmount > maxFeeAmount) revert Errors.IncorrectMaxFeeAmount();

        // Pull all the rewards in this contract
        IERC20(pledgeParams.rewardToken).safeTransferFrom(
            creator,
            address(this),
            totalRewardAmount
        );
    ...
    function increasePledgeRewardPerVote(
        ...
        if (newRewardPerVote <= oldRewardPerVote)
            revert Errors.RewardsPerVotesTooLow();
        ...
        uint256 totalRewardAmount = (rewardPerVoteDiff *
            pledgeParams.votesDifference *
        ...
        emit IncreasePledgeRewardPerVote(
            pledgeId,
            oldRewardPerVote,
            newRewardPerVote
        );
     ...
     */
    function retrievePledgeRewards(uint256 pledgeId, address receiver)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        Pledge storage pledgeParams = pledges[pledgeId];
        if (pledgeParams.endTimestamp > block.timestamp)
            revert Errors.PledgeNotExpired();
        ...
        if (remainingAmount > 0) {
            ...
            IERC20(pledgeParams.rewardToken).safeTransfer(
                receiver,
                remainingAmount
            );
     ...
     * @param receiver Address to receive the remaining rewards
     */
    function closePledge(uint256 pledgeId, address receiver)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        internal
    {
        if (minAmountRewardToken[token] != 0)
            revert Errors.AlreadyAllowedToken();
     ...
     */
    function addMultipleRewardToken(
        address[] calldata tokens,
        uint256[] calldata minRewardsPerSecond
    ) external onlyOwner {
        uint256 length = tokens.length;

        if (length == 0) revert Errors.EmptyArray();
        if (length != minRewardsPerSecond.length)
            revert Errors.InequalArraySizes();
     ...
     * @param minRewardPerSecond Minmum amount of reward per vote per second for the token
     */
    function addRewardToken(address token, uint256 minRewardPerSecond)
        external
        onlyOwner
    {
     ...
     * @param minRewardPerSecond Minmum amount of reward per vote per second for the token
     */
    function updateRewardToken(address token, uint256 minRewardPerSecond)
        external
        onlyOwner
    {
        if (token == address(0)) revert Errors.ZeroAddress();
     ...
     */
    function updateMinTargetVotes(uint256 newMinTargetVotes)
        external
        onlyOwner
    {
    ...
    function recoverERC20(address token) external onlyOwner returns (bool) {
        if (minAmountRewardToken[token] != 0)
            revert Errors.CannotRecoverToken();
